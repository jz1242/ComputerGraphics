...
{
	if( fscanf( fp , " %d %d %d %d " , &x1 , &y1 , &x2 , &y2 )!=4 ) throw( LineSegmentsException( std::string( "Failed to read oriented line segment" ) ) );
}
...
void OrientedLineSegmentPairs::setCount( int count )
{
	...
	if( !_segments ) throw( LineSegmentsException( std::string( "Failed to allocate line segment pairs: " ) + std::to_string( count ) ) );
	_count = count;
}
void OrientedLineSegmentPairs::read( const char* fileName )
{
	FILE* fp = fopen( fileName , "r" );
	if( !fp ) throw( LineSegmentsException( std::string( "Could not open file for reading: " ) + std::string( fileName ) ) );
	try{ _read( fp ); }
	catch( LineSegmentsException e ){ fclose( fp ) ; throw e; }
	fclose( fp );
}
void OrientedLineSegmentPairs::_read( FILE* fp )
{
	...
	if( !fscanf( fp , " %d " , &c ) ) throw LineSegmentsException( std::string( "Failed to read count from file" ) );
	setCount(c);
	try
	{
