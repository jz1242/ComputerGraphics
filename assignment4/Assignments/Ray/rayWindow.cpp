...
Util::Timer  RayWindow::timer;
double       RayWindow::radius;
double       RayWindow::frameRate;
Util::Timer  RayWindow::frameCountTimer;
...
{
	if( !entry ) Util::ErrorOut( "No entry selected" );
}
...
void RayWindow::RayView( RayScene& s , int width , int height , int cplx )
{
	...
	if( glewInit()!=GLEW_OK ) Util::ErrorOut( "glewInit failed" );
