...
GLSLProgram::GLSLProgram( const char* vs_filename , const char* fs_filename ) throw( GLSLProgramException ) : _handle( 0 ) , _linked( false )
{
	// Check that the first shader is a vertex shader and that the file exists. If it does, read in the code.
	if( GetShaderType( vs_filename )!=GL_VERTEX_SHADER ) throw GLSLProgramException( std::string( "Expected vertex shader: " ) + std::string( vs_filename ) );
	...
	if( GetShaderType( fs_filename )!=GL_FRAGMENT_SHADER ) throw GLSLProgramException( std::string( "Expected fragment shader: " ) + std::string( fs_filename ) );
...
GLenum GLSLProgram::GetShaderType( const char* fileName ) throw( GLSLProgramException )
{
...
void GLSLProgram::compileShader( const std::string & source , GLenum type ) throw( GLSLProgramException )
{
	if( _handle<=0 )
	{
		_handle = glCreateProgram();
		if( _handle==0 ) throw GLSLProgramException( "Unable to create shader program." );
	}
...
{
	if( _linked ) return;
	if( _handle<=0 ) throw GLSLProgramException( "Program has not been compiled." );
...
{
	if( _handle<=0 || (!_linked) ) throw GLSLProgramException( "Shader has not been linked" );
...
{
	if( !_linked ) throw GLSLProgramException( "Program is not linked" );
