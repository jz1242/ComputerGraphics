...
public:
	...
	// Ray-Tracing stuff //
	///////////////////////
	/** This virtual method returns the ambient contribution of the light source to the specified hit location. 
	* It computes the amount of amount light reaching the hit location and scales that using the material properties of the hit location.
	* The returned value is a 3D point whose coefficients should all be in the range [0,1].
	*/
	virtual Point3D getAmbient( Point3D cameraPosition , struct RayIntersectionInfo& iInfo )=0;
	...
	/** This virtual method tests if the intersection point represented by iInfo is in shadow from the light source.
	  ...
	  */
	virtual bool isInShadow( struct RayIntersectionInfo& iInfo , class RayShape* shape , int &isectCount )=0;
	...
	/** This method calls the necessary OpenGL commands to render the light.
	  ...
	  */
	virtual void drawOpenGL( int index , GLSLProgram * glslProgram )=0;
