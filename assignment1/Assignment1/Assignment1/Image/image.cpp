...
void Image32::setSize( int width , int height )
{
	if( _w!=width || _h!=height )
	{
		...
		if( !_pixels ) throw( ImageException( std::string( "Failed to allocate memory for image: " ) + std::to_string( width ) + std::string( " x " ) + std::to_string( height ) ) );
	}
	_w = width;
	_h = height;
...
{
	if( x<0 || x>=_w || y<0 || y>=_h ) throw( ImageException( std::string( "Pixel index out of range: ( " ) + std::to_string( x ) + std::string( " , " ) + std::to_string( y ) + std::string( " ) not in [ 0 , " ) + std::to_string( _w ) + std::string( " ] x [ 0 , " ) + std::to_string( _h ) + std::string( " ]" ) ) );
}
...
void Image32::read( char* fileName )
{
	...
	delete[] ext;
	if( !ret ) throw( ImageException( std::string( "Failed to read image: " ) + std::string( fileName ) ) );
}
void Image32::write( char* fileName )
{
	...
	{
		delete[] ext;
		throw( std::string( "Cannot write empty image: " ) + std::string( fileName ) );
	}
	...
	delete[] ext;
	if( !ret ) throw( ImageException( std::string( "Failed to write image: " ) + std::string( fileName ) ) );
}
