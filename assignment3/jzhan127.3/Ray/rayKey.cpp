...
void RayKeyFile::read( char* fileName )
{
	char temp[100];
	...
	int dofs;
	if( fscanf( fp , " #DOFS %d " , &dofs )!=1 || dofs<=0 ){ fclose(fp) ; throw( RayException( std::string( "Failed to parse DOFS in RayKeyFile::read\n" ) ) ); }
	_pNames.resize( dofs );
	_keyTransforms.resize( dofs );
	for( int i=0 ; i<dofs ; i++ )
	{
		if( fscanf( fp , " %s " , temp )!=1 ){ fclose(fp) ; throw( RayException( std::string( "Failed to read DOFS in RayKeyFile::read\n" ) ) ); }
		...
		if( !_pNames[i] )
		{
			...
			_pNames.resize( 0 );
			fclose( fp );
			throw( RayException( std::string( "Failed to allocate parameter names in RayKeyFile::read\n" ) ) );
		}
	...
	if( fscanf( fp , " #DURATION %f " , &duration )!=1 || duration<=0 || fscanf( fp , " #FRAMES %d " , &frames )!=1 || frames<=0 )
	{
		...
		_pNames.resize( 0 );
		fclose(fp);
		throw( RayException( std::string( "Failed to read DUATION in RayKeyFile::read\n" ) ) );
	}
	...
	for( int j=0 ; j<frames ; j++ ) for( int i=0 ; i<dofs ; i++ ) for( int k=0 ; k<4 ; k++ ) for( int l=0 ; l<4 ; l++ )
	{
		...
		if( fscanf( fp , " %f" , &t )!=1 )
		{
			...
			_pNames.resize( 0 );
			fclose( fp );
			throw( RayException( std::string( "Failed to read parameter in RayKeyFile::read\n" ) ) );
		}
		_keyTransforms[i][j](k,l) = t;
	}
	fclose( fp );
}
void RayKeyFile::write( char* fileName )
{
...
void RayKeyData::set( RayKeyFile& keyFile , int tp )
{
	...
	for( int i=0 ; i<_sampleData.size() ; i++ )
	{
		...
		if( !_sampleData[i] || !_sampleData[i]->set( keyFile.samples() ) ){ _clear() ; throw( RayException( std::string( "Failed to set sample data in RayKeyData::set" ) ) ); }
